...
def run_feats(data_dir: Path, schema: Path, out_csv: Path,
    ...
    """
    Вызывает detect_inj/feats.py. Если задан predict_csv — передаём его,
    чтобы посчитать фичи только по нужным файлам.
    ...
    if filelist_csv is not None:
        cmd += ["--filelist_csv", str(filelist_csv), "--filelist_col", filelist_col]
...
# ---------------- main ----------------
def main():
    ap = argparse.ArgumentParser(description="Predict on npy: feats -> XGB model (with predict_csv filtering & metrics)")
    ...
    # новое: CSV со списком файлов и (опц.) метками
    ...
    # подобрать имя колонки с файлами
    ...
    # 1) посчитать фичи только по этим файлам
    ...
    # Оставляем только те фичи, которые есть в списке predict_csv
    F = F.merge(PRED[["_key_base","_key_stem"]].drop_duplicates(), on=["_key_base","_key_stem"], how="inner")
    if F.empty:
    ...
    # поддержка бинарной и мультикласс — но метрики ниже будут рассчитаны только для бинарной
    if proba.shape[1] == 2:
        prob_pos = proba[:, 1]
    ...
    out = pd.DataFrame({
        "filename": F["_key_base"],   # нормализованное имя
        "prob_injury": prob_pos,
        "pred": pred,                 # 1 = injury (для бинарного)
    })
    # вернём «красивое» исходное имя, если оно было в predict_csv
    rev = PRED[[args.file_col, "_key_base"]].drop_duplicates("_key_base").set_index("_key_base")
    out["filename_orig"] = out["filename"].map(rev[args.file_col].to_dict())
    ...
    if label_col and label_col in PRED.columns and proba.shape[1] == 2:
        y_true_raw = PRED[[label_col, "_key_base", "_key_stem"]].copy()
        # нормализация меток
        ...
        # соединяем с предсказаниями по ключу
        M = out.merge(y_true_raw[["_key_base","y"]], left_on="filename", right_on="_key_base", how="inner")
        if len(M) == 0:
